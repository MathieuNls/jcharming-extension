...
\runningheads{Mathieu Nayrolles {\it et al.}}{JCHARMING: A Directed Model Checking Engine Using Crash Traces to Reproduce Bug.}
...
\red{In previous work \cite{Nayrolles2015} we proposed an hybrid approach called JCHARMING
...
\red{In our previous work \cite{Nayrolles2015}, we assessed the effectiveness of using model checking engine directed by stack traces. Indeed, we were capable of of achieving 85\% accuracy
when applied to reproducing bugs from seven different open
source systems.}

\red{In this extension to our previous work, we improve the performances of our approach by further reducing the number of states to explore and apply  JCHARMING to a total of 10 systems (+3), namely Ant, ArgoUML, dnsjava, jfreechart, Log4j, MCT, pdfbox, Mahout, Hadoop and ActiveMQ  and 30 bugs (+10) with an accuracy of 91\%.}


\red{\\In summary, our contributions from \cite{Nayrolles2015} include:}

\begin{itemize}
	\item  \red{A first approach, JCHARMING, for reproducing bug using crash traces and directed model checking.}
	\item \red{A directed model checking engine, based on \cite{Visser2004}, that reduces the number of states to explore in order to reproduce a bug.}
	\item \red{A validation on 7 systems and 20 bugs with an accuracy of 85\%.}
\end{itemize}



\red{\\This paper extends our previous work \cite{Nayrolles2015}, with the following contributions: an extensive validation of JCHARMING on larger scale systems, {\it i.e}, Mahout, Hadoop and, ActiveMQ with 10 more bugs. We further reduce the space search required to reproduce bugs, therefore improving the performances, and improve the accuracy of JCHARMING from 85\% to 91\%. Furthermore, we provide a more elaborated and up-to-date related work.}
...
\red{In this section, we put the emphasis on how stack traces are used in the crash reproduction activity. 
The published works that fold in this category can be divided into two distinct parts: (A) on field record and in house replay \cite{Steven2000,Narayanasamy2005,Artzi2008,Roehm2015} and (B) on house crash explanation \cite{Jin2012,Jin2013,Zuddas2014,Chen2013a,Nayrolles2015}}. 

\red{These two categories yield heterogeneous results depending on the chosen approach and are mainly differentiated by the need of instrumentation. 
Indeed, the first category need to oversee -- by means of instrumentation -- the execution of the targeted system on the field in order to reproduce the crashes in house, while tools and approaches belonging to the second category only use materials produced by the crash such as the crash stack or the core dump at crash time.In the first category, tools record different type of evidence such as invoked methods \cite{Narayanasamy2005}, try-catch exception \cite{Rossler2013} or objects \cite{Jaygarl}. In the second category, the tools and approaches aim to understand what happens only using material produced by the crash itself, such as crash stack \cite{Chen2013a}, previous -- and controlled -- execution \cite{Zuddas2014} or model checking \cite{Nayrolles2015}} 

\red{In one hand, the tools and approaches relying on instrumentation face common limitations such as: (A-1) the needs to instrument the source code in order to introduce logging mechanisms \cite{Narayanasamy2005,Jaygarl,Artzi2008} and therefore (A-2) slowing down the target system. 
In addition to these limitations, (A-3) recording system behavior by means of instrumentation in order to eventually reproduce on-field crashes yield severe privacy concerns. 
On the other hand, tools and approaches that only use material that are the result of a crash -- such as core dump or exception stack crash -- face different limitations.
Indeed, in this second category, approaches have to (B-1)reconstruct the timeline of events that have led to the crash \cite{Chen2013a,Nayrolles2015}. 
Computing all the paths from the initial state of the software to the crash point is a NP-Complex problem and (B-2) approaches struggle with the combinational explosions of states \cite{Chen2013a,Clause2007}}

\red{In order to overcome the complexity of the problems, different authors have used different SMT solver \cite{Dutertre2006} and even model checking techniques \cite{Visser2003}. 
However, these techniques require knowledge that fold far behind classical software engineering and are therefore, (B-3) complex to implement \cite{Visser2004}.}

\red{Finally, both categories share a common limitation. Modern softwares are made to interact with the external world and it is possible for the required condition to reproduce a crash to be purely external (AB-4). For example, the reading of a file only present on the hard drive of the customer or the reception of a faulty network packet \cite{Chen2013a, Nayrolles2015}.}

\subsection{\red{On field record and in house replay}}

\red{Jaygarl {\it et al.} created OCAT (Object Capture based Automated Testing) \cite{Jaygarl}. The authors' approach starts by capturing objects created by the program when it runs on-field in order to provide them to an automated test process. The coverage of automated test is often low due to lack of correctly constructed objects. Also, the objects can be mutated by means of evolutionary algorithms. These mutations target primitive field in order to create even more objects and therefore, improve the code coverage. While not directly targeting the reproduction of a bug, OCAT is an approach that was used as the main mechanism for bug reproduction systems.}

\red{Narayanasamy {\it et al.} \cite{Narayanasamy2005} proposed BugNet, a tool that continuously records program execution for deterministic replay debugging. According to the authors, the size of the recorded data needed to reproduce a bug with high accuracy is around 10MB. This recording is then sent to the developers and allows the deterministic replay of a bug. The authors, argued that with nowadays Internet bandwidth, the size of the recording is not an issue during the transmission of the recorded data. While this approach respects each and every characteristic of the category it is belonging in, it does not provide any responses nor lead to the challenges we have seen in the introduction of this section.}

\red{Another approach of this category was  proposed by Clause {\it et al.} \cite{Clause2007}. The approach records the execution of the program on the client side and compresses the generated data to the minimal required size that ensures that the reproduction is still achievable. This compression is also done on the client side. Moreover, they keep traces of all accessed documents in the operating system and also compress/reduce them to the minimal. Overall, they are able to reproduce on-field bug using a file weighting, averaging 70Kb. The minimal execution paths triggering the failure are then sent to the developer who can replay the execution on a sandbox simulating the client's environment.
This special feature of the approach proposed by Clause et al. [Clause] addresses the AB-4 limitation where crashes are caused by external causes. While the authors broaden the scope of reproducible bug and address the AB-4 limitation, they  are recording a lot of private data (A-3). Indeed, all accessed files are saved to enable the reproduction and the operation of the operating system.}

\red{Timelapse \cite{Burg2013} also address the AB-4 limitation. It focuses on web-application and allows developers to browse and visualize the execution traces recorded by Dolos. Dolos captures and reuses user inputs and network responses to deterministically replay a field crash. Also. both Timelapse and Dolos allow the developers to use conventional tools such as breakpoint and classical debugger. Similarly, private data are recorded without  obfuscation of any sort.}

\red{Another approach proposed by Shay Artzi {\it et al.} and named ReCrash records the object states of the targeted programs \cite{Artzi2008}. They use an in-memory stack which contains every argument and object clones of the real execution in order to reproduce a crash via the automatic generation of unit test cases. 
Unit test cases are used to provide hints to the developers with buggy code. 
This approach particularly suffers from the A-2 limitation related to slowing down the execution. 
Indeed, the overhead for full monitoring is considerably high (between 13\% and 64\% in some cases). 
They also propose an alternative in which they record only the methods surrounding the crash. For this work, the crash has to occur at least once so they could use the information causing the crash to identifying the methods surrounding it.}

\red{Similarly to ReCrash, JRapture \cite{Steven2000} is a capture/replay tool for observation-based testing. The tool captures execution of Java programs to replay it in-house. To capture the execution of a Java program, they used their own version of the Java Virtual Machine (JVM) and employ a lightweight, transparent capture process. 
...
Indeed, instead of instrumenting the source code, they require the user to install their JVM that might have some discrepancies with the original one and yield bugs if used with other software. 
Consequently, JRapture fails at assessing the A-1 and A-3 limitation because it imposes the installation of another JVM that can monitor other software than the intended one.}

\red{RECORE (REconstructing CORE dumps) is a tool proposed by Robler {\it et al.} The tool instruments Java bytecode to wrap every method in a try and catch block while keeping a quasi-null overhead \cite{Rossler2013}. They start from the core dump and try (with evolutionary algorithms) to reproduce the same dump by executing the programs many times. When the generated dump matches the collected one, they have found the set of input responsible for the failure.}

\red{As of now, all presented approaches operate at code level, however, other approaches \cite{Herbold2011,Roehm2015} focus on recording user-GUI interactions. 
Roehm {\it et al.} extract the recorder data using delta debugging \cite{Zeller2002}, sequential pattern mining and their combination to reproduce between 75\% and 90\% of the submitted bug while pruning 93\% of the actions.}

\red{Among approaches presented here, only the ones proposed by Clause {\it et al.} and Burg {\it et al.} address one limitation (AB-4) at the cost, however, of A-3 (privacy). Beside the AB-4 limitation, the RECORE approach has lowered the A-2 limitation (speed). One way of lowering the A-2 and A-3 limitation will be to let user choose where to put the bar between the speed of the targeted program, privacy and bug reproduction efficiency. As an example, users can choose to contribute or not to improving the software -- policy employed by many major players such as Microsoft in Visual Studio or Mozilla in Firefox -- and propose different type of monitoring where the cost in terms of speed, privacy leaks and efficiency at reproducing bug is clearly explained.}

 

\subsection{\red{On house crash explanation}}

\red{On the other side of the picture, we have the tools and approaches belonging to the on house crash explanation are fewer and newer.
...
\red{While being close to our approach, BugRedux and F3 may require the call sequence and/or the complete execution trace in order to achieve bug reproduction. When using only the crash traces (referred to as call stack at crash time in their paper), the success rate of BugRedux significantly drops to 37.5\% (6/16). The call sequence and the complete execution trace required to reach 100\% of bug reproduction can only be obtained through instrumentation and with an overhead ranging from 10\% to 1066\%. Chronicle \cite{Bell2013}, is an approach that supports remote debugging by capturing inputs in the application through code instrumentation. 
They denote themselves by a having a low overhead on the instrumented application, around 10\%.}

\red{Likewise, Zamfir {\it et al.} proposed ESD \cite{Zamfir2010}, an execution synthesis approach which automatically synthesizes failure execution using only the stack trace information. However, this stack trace is extracted from the core dump and may not always contain the components that caused the crash. }

\red{To the best of our knowledge, the most complete work in this category is the one of Chen in his Ph.D thesis \cite{Chen2013a}. Chen proposed an approach named STAR (Stack Trace used based Automatic crash Reproduction). Using only the crash stack, STAR starts from the crash line and goes backward towards the entry point of the program. During the backward process, STAR computes the required condition using an SMT solver named Yices \cite{Dutertre2006}. The objects that satisfy the required conditions are generated and orchestrated inside a JUnit test case. The test is run and the resulting crash stack is compared to the original one. If both match, the bug is said to be reproduced. Star clearly focuses on assessing the B-2 limitation (state explosion). Indeed, they try to reconstruct the events in a backward fashion and therefore save numerous states to explore. Also, Star do well with B-3 (complexity to implement) as it uses Yices \cite{Dutertre2006} and potentially Z3 \cite{de2008z3} (stated in future work) that are well-known and well-supported SMT solver.} \\

\red{Except for STAR, approaches targeting the reproduction of field crashes require the instrumentation of the code or the running platform in order to save the stack call or the objects to successfully reproduce bugs. As we discussed earlier, instrumentation can cause a massive overhead (1\% to 1066\%) while running the system. In addition, data generated at run-time using instrumentation may contain sensitive information.}

\red{JCHARMING's directed model checking overcome the state explosion problem (B-2) of classical model checking and allow the generation of Junit test case in a reasonable amount of time. 
In order to address the limitation B-1 (reconstruct the events), B-2 (state explosion) and B-3 (complex implementation) authors have adopted different approaches such as execution synthesis, backward analysis, well-known and supported SMT and directed model checking, respectively. While all approaches assess B-1 and are able to yield acceptable results, it seems that no good trade-off can be found between the simplicity of implementation (B-3) and the reduction of state explosion (B-2).}
...
For industrial size system, this trace can contain mullions of instructions. This trace is stored in memory and therefore can be queried while JPF is running. 
...
Each time on new instruction is processed by JPF, our listener catch it and then save it to a mongodb database to be queried post-mortem. Figure \ref{fig:jcharming-unittest} presents a high level architecture of the different components.}

\begin{figure}[h!]
  \centering
    \includegraphics[scale=0.8]{media/unittest.png}
    \caption{\red{High level architecture of the Junit test case generation}
    \label{fig:jcharming-unittest}}
\end{figure}


\red{When the {\tt validate} step trigger a crash stack similarity superior to a factor {\tt t}, the Junit generation engine queries the mongoDB database and fetches the sequence of instructions that led to the crash of interest. 
...
From this sequence we know that, to reproduce to crash of interest, we have to (1) create new object {\tt jsme.Bar} ({\tt new jsme.Bar}, {\tt invokespecial jsme.Bar()}), (2) store the newly created object in a variable named {\tt bar} ({\tt astore\_1 [bar]}), (3) invoke the method {\tt jsme.Bar.foo(int)} of the {\tt bar} object with {\tt 3} as value ({\tt aload\_1 [bar]}, {\tt iconst\_3}, {\tt invokevirtual jsme.Bar.foo(int)}). Then, the {\tt jsme.Bar.foo(int)} method will executes a loop from 0 to 3 and throw an exception at {\tt i = 3}~({\tt const\_0}, {\tt istore\_2 [i]}, {\tt goto}, {\tt iload\_2 [i]}, {\tt iconst\_2}, {\tt if\_icmple iinc 2 1}, {\tt new java.lang.Exception}).}  

\red{The generation of the JUnit test itself is based on templates and targets directly the system under test. Templates are excerpts of Java source code with well-defined tags that will be replaced by values. We use templates because the JUnit test case have common structures as shown by Figure \ref{fig:jcharming-unittemplate}.}
...
\noindent\fbox{%
    ...
    \caption{\red{Simplified Unit Test template}
