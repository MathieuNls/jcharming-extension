...
\red{In a previous work, we proposed a hybrid approach called JCHARMING
...
the content of the heap either. \red{Instead, JCHARMING uses a
...
\red{All the presented approaches so far operate at code level.
...
\red{BugRedux relies on several in-house executions that are synthesized in order to narrow down the search scope,  find the crash location, and finally reproduce the bug.}
However, these in-house executions have to be conducted prior the work on the bug really begins.
\red{Also, the in-house executions suffer from the same limitation as unit testing {\it i.e.}, the executions are based on the developer's knowledge and abilities to develop unlikely scenarios and not only normal executions.}
...
\red{where S is the set of states, $T \subseteq S * S$ represents the transitions
...
\red{If there exists path contradicting $p$ (i.e., $\exists x$  such that $(SUT, x)
...
\red{Figure \ref{fig:jcarming-traces} shows an example of a crash trace, respecting
...
final slice. \red{In the rare cases where the final slice is empty (this
