...
open source programs. Overall, JCHARMING is able to reproduce 80\% of the bugs used in this study in an average time of 19 minutes.
...
required for JCHARMING to fill out all the available memory and stop.
...
running a full model checker. It is difficult to evaluate the number of times we may face this situation without conducting an empirical study in the quality of crash traces. We defer this to future work.
...
directed model checking. JCHARMING relies on crash traces and backward program slices to direct a model checker. This way, we do not need to visit all the states of the subject program, which would be computationally taxing. When applied to .  When applied to thirty bugs from six open source systems, JCHARMING was able to reproduce 30 bugs of 10 open source systems, JCHARMING achieved an 80\% accuracy. The average time to reproduce a bug was 19 minutes, which is quite reasonable, given the complexity of reproducing bug that cause field crashes. 
