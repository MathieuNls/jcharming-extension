...
Such crash traces are imcompled as they do not show all the method calls to go from the entry point of the program to the crash point.}
...
There exists many techniques to perform static analysis (data flow analysis, control flow analysis, theorem proving, ect\ldots) that can be used to improve programs in terms of performances, understandability or to debug them.
One of the product of static analysis is the static slice which takes a program and slicing properties in order to create a smaller program with respect to the slicing properties.
this is particularly interesting for JCHARMING as the sliced program, being smaller, will have a smaller state space.
In Figure \ref{fig:slicing}, black states are states that are impacted by the slacing properties.
...
\red{The advantages of computing backward static slice over or static slice are twofold. First, with the stack trace, we have the location of the statement that yield the crash (\textit{n}) and entry point of the program (\textit{m}).} Second, for large systems, a crash trace does not necessarily contain all
...
algorithm.
Namely, our algorithm needs to have the crash stack as an array of frame (line 1), the size $n$ of the crash stack (line 2) a \texttt{null} backward static slice (line 4) and an offset set to 1 (line 3).
...
However, $f_3$ has been tempered with and no longer match a location inside the SUT.
...
At this point, we passed throught the $for$ loop (line 5 to 12) two times and both times, the backward static slice was not empty.
...
Unfortunately, $f_3$ is corrupted and do not point towards a valide location in the SUT.
...
\red{Our algorithm, given an uncorupted stack trace will be able to compute an optimum backward static slice based on the frames (Figure~\ref{fig:jcharming-slice}) and compensate for frames corruption if needs be by \texttt{offsetting} the corrupted frame (Figure~\ref{fig:jcharming-algo}).
The compensation of corrupted frames, obviously, comes at the cost of a sub-optimum backward static slice.
In our previous example, the transition between $b$ and $h$ could have been ommited if $f_3$ was not corrupted.
}
...
and, thereofore, allow the whole Java language to be model checked. Its consists of a custom JVM --- known as JVM\textsuperscript{\textit{JPF}} --- that executes the bytecode instructions.
...
Furthermore, JPF is an explicite state model checker
...
Currently\footnote{JPF is still under development at the NASA Bablefish lab.}, the JPF model checker is able to check deadlocks, invariants and user-defined code assertion as same as LTL (Linear Time Logic) properties.}
...
\begin{itemize}
  \item \red{The informations of each thread. More specifically, a stack of frames corresponding to methods' calls.}
