...
In a previous work, we proposed a hybrid approach called JCHARMING
...
list of function outputs when an uncaught exception in Java
...
Using their own JVM allows capturing any interactions between a Java program and the system including GUI, files, and console inputs.
On the replay these interactions  with exactly the same input sequence as saw during the capture phase.
...
All the presented approaches so far operate at code level.
There also exist  approaches \cite{Herbold2011,Roehm2015} that focus on recording user-GUI interactions.
...
BugRedux relies on several in-house executions that are synthesized in order to narrow down the search scope,  find the crash location, and finally reproduce the bug.
...
Model checking (also known as property checking) will, given a formally defined system
...
where S is the set of states, $T \subseteq S * S$ represents the transitions
...
However, this only ensures that $\exists x$ such that $p$ is reached
at some point in the execution of the program and not that $p$ holds
nor that $\forall x$, $p$ is satisfiable.
In JCHARMING, SUTs are bound to a simple assumption:
they must not crash under a fair environment.
In the framework of this study, we consider a fair environment
...
If there exists path contradicting $p$ (i.e., $\exists x$  such that $(SUT, x)
...
Figure \ref{fig:jcarming-traces} shows an example of a crash trace, respecting
the Java grammar for Stack Traces \cite{LucaDallOlioChristianCassano2008}, that contains the
...
that are beyond control. Frames out of our control refer
usually, but not necessarily, Java library methods and third
...
transition between each two frames, which is very unlikely for real
...
algorithm. The main loop of the algorithm begins at line 5 and
ends at line 13. In this loop, we compute the static slice
...
$t$ is a threshold specified by the user. Thus, $t$ is the percentage of
...
For industrial sized system, this trace can contain millions of instructions. This trace is stored in memory and therefore can be queried while JPF is running.
...
In order to allow JPF to use 100\% of the available resources and still be able to query the executed instruction post-mortem, we implemented a {\tt listener} that listens to the JPF trace emitter.
...
The generation of the JUnit test itself is based on templates and targets directly
...
the {\tt differences} which count how many lines differ between the orginal
...
Then, the stack trace related to the crash is received in {\tt catch} part of
the {\tt try-catch} block. In the {\tt catch} part, we compute the number of lines
that does not match the original exception and store it into {\tt differences}
...
the stack traces from the induced and the original crash are at least {\tt threshold}
...
\footnote{The bug reports used in this study and the result of JCHARMING are
made available for download from research.mathieu\-nayrolles.com/jcharming/}. The aim of the
...
ArgoUML \cite{CollabNet} is one of the major players among the open source
...
Mahout \cite{mahout2012scalable} is a relatively new software application, built on top of Hadoop. We used Mahout version 0.11, which was released in August 2015.
...
While this quick comment, made only two and a half hours after the bug submission, was insightful as shown in our generated test case\footnote{As a reminder, the generated test cases are made available at research.mathieu-nayrolles.com/jcharming}, the fix happened in the {\tt CollocCombiner} class which is one of the {\tt Reducer}\footnote{As in Map/Reduce} available in Mahout.
...
The resolution of this bug\footnote{https://github.com/apache/mahout/pull/38\#issuecomment-51436303}
...
Finally, the programs we used in this study are all written in
