...
\begin{abstract}
\red{Due to their inherent complexity, software systems
...
open source programs cumulating more than one and a half million lines of
code scattered in around 17,500 classes. Overall, JCHARMING
was able to reproduce 80\% of the submitted bugs.}
\end{abstract}
...
exceptions. Then, with the aim to guide the directed model checking engine in an optimal way, we remove frames
...
\red{Hadoop \cite{hadoop2011hadoop} is a framework for storing and processing large dataset in distributed environments. It contains four main modules: {\it Common}, {\it HDFS}, {\it YARN} and {\it MapReduce}. In this paper, we study the {\it Common} module that contains the different libraries required by the other three modules.}

\red{Mahout \cite{mahout2012scalable} is a relatively new piece of software, built on top of Hadoop that does not have reach its first version\footnote{version 0.11 was released 7$^{th}$ of august 2015}. 
Mahout focuses on producing free implementation of machine learning algorithms primarily in the areas of collaborative filtering, clustering and classification.}

\red{Finally, ActiveMQ \cite{snyder2011activemq} is an open source messaging server that allows application written in Java, C, C++, C\#, Ruby, Perl or PHP to exchange messages using various protocols. ActiveMQ is actively maintained and developed since it became an Apache Software Foundation project in 2005.}
...
\multirow{2}{*}{PDFBox}     & 1412     & Partial & 19.7     & -       \\
                            & 1359     & No      & -        & - \\ \hline
\multirow{4}{*}{\red{Mahout}}   & 486      & Partial & 34.5     & -        \\
                            & 1367     & Partial & 21.1     & -       \\ 
                            & 1594 	   & No 	 & - 	    & -       \\
                            & 1635     & Yes     & 31.0     & - \\ \hline 
\multirow{3}{*}{\red{Hadoop}}     & 2893     & Partial & 7.4      & 32       \\
						    & 3093     & Yes 	 & 13.1     & -        \\
                            & 11878    & Yes 	 & 17.4     & - \\  \hline 
\multirow{3}{*}{\red{ActiveMQ}}   & 1054     & Yes & 38.3     & -      \\
						    & 2880     & Partial 	 & -        & -        \\
                            & 5035     & No 	 & 27.4     & - \\  \hline \hline             
\end{tabular}
...
out of 30 bugs either completely or partially (80\% success
ratio). The average time to reproduce a bug is 19 minutes.
...
\red{The second reproduced bug we describe during this discussion is the bug \#486 belonging to MAHOUT. The submitter (Robin Anil) named his entry {\it Null Pointer Exception running DictionaryVectorizer with ngram=2 on Reuters dataset} and simply past the following crash stack without further explanation:}
...
22 at org.apache.hadoop.mapred.Task\$NewCombinerRunner\$OutputConverter.write(Task.java:1201) \\
23 at org.apache.hadoop.mapreduce.TaskInputOutputContext.write(TaskInputOutputContext.java:80) \\
...
\red{Drew Farris, which was assigned to fix this bug, commented {\it Looks like this was due to an improper use of the Gram default constructor that happened as a part of the 0.20.2\footnote{Farris certainly meant 0.10.2 which was the last refactor of the incriminated class and the current version of Mahout is 0.11} refactoring work.} 
While this quick comment, made only two and a half hour after the bug submission was insightful as shown in our generated test case\footnote{As a reminder, generated test cases are made available at research.mathieu-nayrolles.com/jcharming} the fix happen in the {\tt CollocCombiner} class which is one of the {\tt Reducer}\footnote{As in Map/Reduce} available in Mahout. 
...
belong to a second exception thrown because a {\tt java.lang.NullPointerException} happened when writing in a {\tt ByteArrayOutputStream}. 
JCHARMING aims to reproduce the root exceptions and not the exceptions that derive from other exceptions. 
Consequently, the {\tt java.io.IOException: Spill failed} is ignored and our directed model checking engine focused, with success, on reproducing the {\tt java.lang.NullPointerException}.}
...
\red{The second partially reproduced bug we present here is bug \#2893 belonging to HADOOP. This bug, reported in February 2008 by Lohit Vijayarenu as for title {\it checksum exceptions on trunk} and contains the following description: {\it While running jobs like Sort/WordCount on trunk I see few task failures with ChecksumException
...
\red{Similarly to the first partially reproduced bug, the crash traces produced by our directed model checking engine and the related test case did not match the attached crash stack a 100\%. 
