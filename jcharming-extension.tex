...
Automatic Reproduction by directed Model checkING), uses information found in crash traces combined with static program slices to guide a model checking engine in an optimal way. We show that JCHARMING is efficient in reproducing   
We show the effectiveness of JCHARMING by applying it to ten different
open source programs. Overall, JCHARMING is able to reproduce 80\% of the bugs used in this study in average time of 19 minutes.
...
Bug reproduction is however a challenging task because the data provided by the end users is often scarce. Existing techniques can be divided into two categories: (a)
...
exceptions. \red{Nested exception refers to the following structure in Java: \\}
...
\red{In such a case, we want to reproduce the root exception (line 28) 
that led to the the other two (lines 14 and 1). Consequently, we remove the line
1 to 14.}
...
\red{We did not have to extend WALA core in any way to perform our 
analysis, we used it as-is.}
...
\red{For our experiments (see Section \ref{sec:results}) we set the value of $t$ to 80\%.
The choice of this value does not follow any
specific indication, general law about directed model 
checking or bug reproduction. 
We were only guided by the need to find the best trade-off
between the reproducibility and
the pertinence of the generated test case, i.e, does the tests helps
to reproduce the on-field crash or not.}

\red{Thus, we have made several incremental attempts, starting from 10\%
For each attempt, we modified $t$ by 5\% and
observed the number of bug reproduced and the quality of the generated tests.
$t = 80\%$ seamed to be the best trade-off, henceforth, if the user is not informed enough to 
provide a threshold of his own, this value will be used. Moreover, we prevent JCHARMING to perform directed model checking with a threshold below 50\%.
Indeed, tests generated with such a low threshold do not yield qualitative result at all.}
...
ratio).  The average time to reproduce a bug was 19 minutes 
\red{ and the average time to failure was 11 minutes. The time to fail is lower than the time
to reproduce as, in the majority of the failures, the problem was at the $f_0$ level. 
Henceforth, JCHARMING do not try to reproduce. The maximum fail time was 42.3 minutes, which the time
...
\red{The second bug we did not manage to reproduce and 
present here belong to mahout and has been reported by  
Jaehoon Ko on July 2014. The bug \#1594 was entitled {\it Example factorize-movielens-1M.sh does not use HDFS}
...
\red{This entry was ultimately marked as ``Not A Problem'' / ``WONT\_FIX'' meaning that 
the reported bug was not a bug at all. 
Indeed, the resolution\footnote{described here https://github.com/apache/mahout/pull/38\#issuecomment-51436303} 
involved the modification of a bash script Ko was using to {\it query} Mahout. Henceforth, the cause of the failure was external to Mahout itself and JCHARMING could not reproduce it.}
